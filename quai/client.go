...
import (
	...
	quairpc "github.com/dominant-strategies/go-quai/rpc"
...
func NewClient(url string, params *params.ChainConfig, skipAdminCalls bool) (*Client, error) {
	c, err := quairpc.DialHTTPWithClient(url, &http.Client{
...
func (ec *Client) getUncles(
	...
	if len(body.UncleHashes) > 0 {
		...
		reqs := make([]quairpc.BatchElem, len(body.UncleHashes))
		for i := range reqs {
			reqs[i] = quairpc.BatchElem{
...
func (ec *Client) getBlockReceipts(
	...
	reqs := make([]quairpc.BatchElem, len(txs))
	for i := range reqs {
		reqs[i] = quairpc.BatchElem{
